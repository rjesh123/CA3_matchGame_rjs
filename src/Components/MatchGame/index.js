...
class MatchGame extends Component {
  ...
  clearIntervalId = () => clearInterval(this.timeId)
  ...
  restartGame = () => {
    this.setState(prevState => ({isGameStarted: !prevState.isGameStarted}))
    this.setState({score: 0, time: 60, activeShownImage: imagesList[0]})
  ...
  gameOverCard = () => {
    ...
    return <GameOver score={score} playAgain={this.restartGame} />
  }

  onActiveThumbNail = tId => {
    const {activeShownImage} = this.state
    const {id} = activeShownImage
  ...
  renderImagesList = () => {
    const {activeTabId, activeShownImage} = this.state
    const {imageUrl} = activeShownImage
    const filteredImagesList = this.onFilterImagesList()
        ...
        <ul className="thumbnails-container">
          {filteredImagesList.map(eachImage => (
            <ThumbNails
              key={eachImage.id}
              details={eachImage}
              onActiveThumbNail={this.onActiveThumbNail}
            />
          ))}
        </ul>
      </div>
  ...
  render() {
    const {score, time, isGameStarted} = this.state
    return (
      <div className="app-container">
        <Navbar score={score} time={time} />
        <div className="game-card-container">
          {isGameStarted ? this.renderImagesList() : this.gameOverCard()}
        </div>
      </div>
    )
  }
